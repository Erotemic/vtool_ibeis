...
def gridsearch_image_function(param_info, test_func, args=tuple(), show_func=None):
    ...
    cfgresult_list = [
        test_func(*args, **cfgdict) for cfgdict in ub.ProgIter(cfgdict_list, desc=lbl)
    ]
    onclick_func = None
...
def blend_images_average(img1, img2, alpha=0.5):
    r"""
    Args:
...
def blend_images_mult_average(img1, img2, alpha=0.5):
    r"""
    Args:
        img1 (ndarray[uint8_t, ndim=2]):  image data
        img2 (ndarray[uint8_t, ndim=2]):  image data
    ...
    mult_ave = blend_images_multiply(img1_, img2_, 0.5)
    if alpha < 0.5:
        imgB = blend_images_average(img1_, mult_ave, alpha * 2)
    else:
        imgB = blend_images_average(mult_ave, img2_, (alpha - 0.5) * 2)
    # assert imgB.min() >= 0 and imgB.max() <= 1
    return imgB
...
def gridsearch_addWeighted():
    ...
    def test_func(src1, src2, alpha=1.0, **kwargs):
        ...
        cv2.addWeighted(
            src1=src1, src2=src2, dst=dst, alpha=alpha, beta=beta, dtype=-1, **kwargs
        )
        return dst
    ...
    param_info = ut.ParamInfoList(
        'blend_params',
        [
            ut.ParamInfo('alpha', 0.8, varyvals=np.linspace(0, 1.0, 5).tolist()),
            # ut.ParamInfo('beta', .8,
            ut.ParamInfo('gamma', 0.0, varyvals=np.linspace(0, 1.0, 5).tolist()),
            # varyvals=[.0],))
