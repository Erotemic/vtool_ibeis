...
def get_transforms_from_patch_image_kpts(kpts, patch_shape, scale_factor=1.0):
    ...
    M_list = reduce(np.matmul, (S2, invVR_aff2Ds, S1.dot(T1)))
    return M_list
...
def transform_kpts_to_imgspace(kpts, bbox, bbox_theta, chipsz):
    ...
    invCinvV_mats = np.matmul(invC, invV_mats)
...
def transform_kpts(kpts, M):
    ...
    """
    invVR_mats3x3 = get_invVR_mats3x3(kpts)
    MinvVR_mats3x3 = np.matmul(M, invVR_mats3x3)
    try:
...
def get_invVR_mats_oris(invVR_mats):
    r"""
    extracts orientation from matrix encoding, this is a bit tricker
...
def get_Z_mats(V_mats):
    ...
    Z_mats = np.matmul(Vt_mats, V_mats)
    return Z_mats
...
def get_uneven_point_sample(kpts):
    ...
    ellipse_pts1 = np.matmul(invV_mats, circle_pts.T).transpose(0, 2, 1)
    return ellipse_pts1
